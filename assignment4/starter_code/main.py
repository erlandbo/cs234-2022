...
class ClinicalDosingPolicy(StaticPolicy):
	def choose(self, x):
		...
		x_features = np.array([x[f] for f in features])
		weights = np.array([-0.2546, 0.0118, 0.0134, -0.6752, 0.4060, 0.0443, 1.2799, -0.5695])
		weekly_dose = (x_features @ weights + 4.0376)**2 	
...
class LinUCB(BanditPolicy):
	def __init__(self, n_arms, features, alpha=1.):
		...
		self.alpha = alpha
		self.A = np.array([np.eye(len(features)) for _ in range(n_arms)])  # (A, D, D)
		self.b = np.array([np.zeros((len(features), 1)) for _ in range(n_arms)])  # (A, D, 1)
		#######################################################
		#########          END YOUR CODE.          ############
	...
	def choose(self, x):
		...
		x_features = np.array([x[f] for f in self.features])  # (D,)
		theta = (np.linalg.inv(self.A) @ self.b).squeeze(-1)  # (A, D, D) @ (A, D, 1) -> (A,D,1) -> (A, D)
		# p = theta.T @ x_features + self.alpha * (x_features.T @ np.linalg.inv(self.A) @ x_features) ** (1/2)
		# for single action: (1, D) @ (D,1) + (1,D) @ (D,D) @ (D,1) -> (1,1) + (1,1) -> (1,1)
		p = theta @ x_features + self.alpha * (x_features @ np.linalg.inv(self.A) @ x_features) ** (1/2)
		# (A,D) @ (D,) + (D,) @ (A,D,D) @ (D,) -> (A,) + (A,D) @ (D,) -> (A,) + (A,) -> (A,)
		action = np.argmax(p)
		return ['low', 'medium', 'high'][action]
		#######################################################
		######### 
	...
	def update(self, x, a, r):
		...
		x_features = np.array([x[f] for f in self.features])  # (D,)
		self.A[a] += x_features[:, None] @ x_features[:, None].T
		self.b[a] += r * x_features[:, None]
		#######################################################
		#########          END YOUR CODE.          ############
...
class eGreedyLinB(LinUCB):
	...
	def choose(self, x):
		...
		x_features = np.array([x[f] for f in self.features])  # (D,)
		theta = (np.linalg.inv(self.A) @ self.b).squeeze(-1)  # (A, D, D) @ (A, D, 1) -> (A,D,1) -> (A, D)
		p = theta @ x_features[:, None]
		action = np.argmax(p) if np.random.random() > epsilon else np.random.randint(len(actions))
...
class ThomSampB(BanditPolicy):
	...
	def choose(self, x):
		...
		x = np.array([x[f] for f in self.features])  # (D,)
		B_inv = np.linalg.inv(self.B)
		mu_hat = list((B_inv @ self.f[...,None]).squeeze(-1))  # (A,D,D) @ (A,D,1)) -> (A,D,1)-> (A,D)
		sig_hat = list(self.v2 * B_inv)  # (A, D, D)
		mu_tilde = np.array([np.random.multivariate_normal(mu,sig) for mu, sig in zip(mu_hat, sig_hat)])  # (A,D)
		best_action = np.argmax(mu_tilde @ x)
		return ['low', 'medium', 'high'][best_action]
		#######################################################1
		#########          END YOUR CODE.          ############
	...
	def update(self, x, a, r):
		...
		x = np.array([x[f] for f in self.features])  # (D,)
		a = ('low', 'medium', 'high').index(a)
		self.B[a] += np.outer(x, x)  # (D,D) + (D,D) -> (D,D)
		self.f[a] += x * r
		#######################################################
		#########          END YOUR CODE.          ############
