...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        pad1 = ((4 - 1) * img_height - 4 + 8) // 2
        pad2 = ((2 - 1) * img_height - 2 + 4) // 2
        pad3 = ((1 - 1) * img_height - 1 + 3) // 2
        self.q_network = nn.Sequential(
            nn.Conv2d(in_channels=n_channels*self.config.state_history, out_channels=32, kernel_size=8, stride=4, padding=pad1),
    ...
    def get_q_values(self, state, network):
        ...
        if network == 'q_network':
            out = self.q_network(torch.permute(state, dims=(0,3,1,2)))
        else:
            out = self.target_network(torch.permute(state, dims=(0,3,1,2)))
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
