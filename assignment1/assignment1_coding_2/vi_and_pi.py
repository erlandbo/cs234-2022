	...
	while True:
		...
		for state in range(nS):
			...
			for (probability, nextstate, reward, terminal) in P[state][action]:
				Q += probability * (reward + gamma * prev_value_function[nextstate])
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	while True:
		...
		if not np.all(policy == new_policy): policy_stable = False
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for state in range(nS):
			for action in range(nA):
				...
				value_function[state] = np.maximum(Q, value_function[state])
		if np.max(np.abs(value_function-prev_value_function)) < tol: break
	...
	for state in range(nS):
		...
		for action in range(nA):
			for (probability, nextstate, reward, terminal) in P[state][action]:
				Q[action] += probability * (reward + gamma * value_function[nextstate])
