...
def build_mlp(input_size, output_size, n_layers, size):
    ...
    modules = [nn.Linear(in_features=input_size, out_features=size, device=device), nn.ReLU()]
    modules += [nn.Linear(in_features=size, out_features=size, device=device), nn.ReLU()]
    modules += [nn.Linear(in_features=size, out_features=output_size, device=device)]
