...
# from assignment3.code.general import batch_iterator
...
class BaselineNetwork(nn.Module):
    ...
    def __init__(self, env, config):
        ...
        self.baseline = build_mlp(observation_dim, 1, self.config.n_layers, self.config.layer_size)
    ...
    def calculate_advantage(self, returns, observations):
        ...
        baselines = self.forward(observations).cpu().detach().numpy()
    ...
    def update_baseline(self, returns, observations):
        ...
        """N = returns.shape[0]
        batch_it = batch_iterator(returns, self.forward(observations), batch_size=64)
        for _ in range(N//64):
            r, b = next(batch_it)
            self.optimizer.zero_grad()
            loss = nn.functional.mse_loss(r, b)
            loss.backward()
            self.optimizer.step()"""
        self.optimizer.zero_grad()
